  ...
  servers.each do |server|
    ...
    config.vm.define server[:name], autostart: server[:autostart] do |box|
      ...
      if "#{server[:name]}" == 'k8s-demo'
        ...
        box.vm.provision "shell", privileged: true, inline: "kubectl create namespace my-app"
        box.vm.provision "shell", inline: "cd /ansible; ansible-playbook -i hosts.yml auto-scaling.yml"
        box.vm.provision "shell", inline: "cd /ansible; ansible-playbook -i hosts.yml ci.yml"
        box.vm.provision "shell", inline: "cd /ansible; ansible-playbook -i hosts.yml dlppkidev.yml"
      end
